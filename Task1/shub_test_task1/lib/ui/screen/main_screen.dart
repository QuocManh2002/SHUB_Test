...
class _MainScreenState extends State<MainScreen> {
  final TimePickerUtils timePickerUtils = TimePickerUtils();
                    ...
                    BlocBuilder<MainBloc, MainState>(
                      ...
                      builder: (context, state) {
                        return Expanded(
                          child: InkWell(
                            onTap: () async {
                              final result = await timePickerUtils.pickTime(
                                  context, state.startTime ?? TimeOfDay.now());
                              if (result != null) {
                                // ignore: use_build_context_synchronously
                                context
                                    .read<MainBloc>()
                                    // ignore: use_build_context_synchronously
                                    .add(SelectStartTime(time: result));
                              }
                            },
                            child: Container(
                              padding: const EdgeInsets.symmetric(vertical: 15),
                      ...
                      builder: (context, state) {
                        return Expanded(
                          child: InkWell(
                            onTap: () async {
                              final result = await timePickerUtils.pickTime(
                                  context, state.endTime ?? TimeOfDay.now());
                              if (result != null) {
                                // ignore: use_build_context_synchronously
                                context
                                    .read<MainBloc>()
                                    // ignore: use_build_context_synchronously
                                    .add(SelectEndTime(time: result));
                              }
                            },
                            child: Container(
                              padding: const EdgeInsets.symmetric(vertical: 15),
